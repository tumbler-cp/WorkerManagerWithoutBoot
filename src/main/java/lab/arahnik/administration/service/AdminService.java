...
public class AdminService {
  ...
  public AdminRequestElem updateAdminRequest(AdminRequestElem adminRequestElem) {
    var request = adminRequestRepository
            .findByUserId(adminRequestElem.getUserId())
            .orElseThrow(
                    () -> new EntityNotFoundException("User with id " + adminRequestElem.getUserId() + " not found"));
    ...
    if (request.getStatus() == AdminRequestStatus.ACCEPTED) {
      var user = userRepository
              .findByUsername(request
                      .getUser()
                      .getUsername())
              .orElseThrow(() -> new UsernameNotFoundException(request
                      .getUser()
                      .getUsername()));
